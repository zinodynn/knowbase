...
async def list_kb_permissions(
    ...
    result = await db.execute(
        select(func.count(UserKBPermission.id))
    ...
    for perm, username, email in rows:
        items.append(PermissionWithUserResponse(
            id=perm.id,
...
async def create_kb_permission(
    ...
    # 检查目标用户是否存在
    result = await db.execute(
        select(User).where(User.id == perm_in.user_id)
    )
    ...
    if target_user.id == kb.owner_id:
        raise HTTPException(
            ...
            detail="不能给知识库所有者添加权限"
        )
    
    # 检查是否已存在权限
    result = await db.execute(
        select(UserKBPermission).where(
            UserKBPermission.user_id == perm_in.user_id,
...
async def update_kb_permission(
    ...
    result = await db.execute(
        select(UserKBPermission).where(
            UserKBPermission.user_id == user_id,
            UserKBPermission.kb_id == kb_id
        )
    )
    ...
    # 更新权限
    permission.permission = perm_in.permission
    await db.commit()
    await db.refresh(permission)
