...
from app.models.knowledge_base import KnowledgeBase, KBTag
...
async def check_kb_permission(
    ...
    # 公开知识库允许读取
    if kb.is_public and required_permission == "read":
        return kb
    ...
    if permission_levels.get(permission.permission, 0) < permission_levels.get(required_permission, 0):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="权限不足"
        )
...
async def create_knowledge_base(
    ...
    kb = KnowledgeBase(
        ...
        embedding_dimension=kb_in.embedding_dimension,
    ...
    if kb_in.tags:
        for tag_name in kb_in.tags:
            tag = KBTag(kb_id=kb.id, name=tag_name)
            db.add(tag)
...
async def update_knowledge_base(
    ...
    if kb_in.tags is not None:
        # 删除旧标签
        await db.execute(
            select(KBTag).where(KBTag.kb_id == kb_id)
        )
...
@router.get("/{kb_id}/stats", response_model=KnowledgeBaseStats, summary="获取知识库统计")
